...
from src.dataloader import resize_volume, generate_fake_data_coarse, generate_fake_data_fine, generate_real_data, generate_real_data_random, load_real_data
import argparse
import time
import os
    ...
    for k in range(n_epochs):
        for i in range(bat_per_epo):
          ...
          for j in range(2):
              ...
              [X_realA, X_realB], [y1,y2] = generate_real_data(dataset, i, n_batch, n_patch)

              
              # generate a batch of fake samples for Coarse Generator

              X_realA_half = resize_volume(X_realA,int(X_realA.shape[1]/2),
                                           int(X_realA.shape[2]/2),
                                           int(X_realA.shape[3]/2))
              X_realB_half = resize_volume(X_realB,int(X_realA.shape[1]/2),
                                           int(X_realA.shape[2]/2),
                                           int(X_realA.shape[3]/2))

              print(X_realA_half.shape)
              [X_fakeB_half, x_global], y1_coarse = generate_fake_data_coarse(g_global_model, X_realA_half, n_patch)


              # generate a batch of fake samples for Fine Generator
              X_fakeB, y1_fine= generate_fake_data_fine(g_local_model, X_realA, x_global, n_patch)
              ...
              # update discriminator for real samples
              d_loss3 = d_model2.train_on_batch([X_realA_half, X_realB_half], y2)[0]
              # update discriminator for generated samples
              d_loss4 = d_model2.train_on_batch([X_realA_half, X_fakeB_half], y1_coarse)[0]
          ...
          [X_realA, X_realB], _ = generate_real_data(dataset, i,n_batch, n_patch)

          # Global Generator image fake and real
          out_shape = (int(X_realA.shape[1]/2),int(X_realA.shape[2]/2),int(X_realA.shape[3]/2))
          X_realA_half = resize_volume(X_realA,int(X_realA.shape[1]/2),
                                       int(X_realA.shape[2]/2),
                                       int(X_realA.shape[3]/2))
          X_realB_half = resize_volume(X_realB,int(X_realA.shape[1]/2),
          ...
          gan_loss,_,_,fm1_loss,fm2_loss,_,_,g_global_recon_loss, g_local_recon_loss = gan_model.train_on_batch([X_realA,X_realA_half,x_global,X_realB,X_realB_half],
                                                                                                                                                      [y1, y2,
                                                                                                                                                        X_fakeB,X_fakeB_half,
                                                                                                                                                        X_fakeB_half,X_fakeB,
                                                                                                                                                        X_fakeB_half,X_fakeB])
...
if __name__ == "__main__":
    ...
    image_shape_coarse = (in_size//2,in_size//2,in_size//2,4)
    #mask_shape_coarse = (in_size//2,in_size//2,in_size//2,1)
    label_shape_coarse = (in_size//2,in_size//2,in_size//2,1)


    image_shape_fine = (in_size,in_size,in_size,4)
    #mask_shape_fine = (in_size,in_size,in_size,1)
    label_shape_fine = (in_size,in_size,in_size,1)
    
    image_shape_xglobal = (in_size//2,in_size//2,in_size//2,128)
    ndf=64
    ncf=128
    nff=128
    ...
    g_model_fine = fine_generator(x_coarse_shape=image_shape_xglobal,input_shape=image_shape_fine,nff=nff,n_blocks=3)
    g_model_coarse = coarse_generator(img_shape=image_shape_coarse,n_downsampling=2, n_blocks=9, ncf=ncf,n_channels=1)
    ...
    rvgan_model = RVgan(g_model_fine,g_model_coarse, d_model1, d_model2,
                  image_shape_fine,image_shape_coarse,image_shape_xglobal,label_shape_fine,label_shape_coarse,args.inner_weight)
