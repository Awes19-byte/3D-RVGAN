...
from tensorflow.keras.layers import Layer, InputSpec, Reshape, Activation, Conv3D, Conv3DTranspose, SeparableConv2D, Dropout
from tensorflow.keras.layers import Input, Add, Concatenate, Lambda,LeakyReLU,AveragePooling2D, BatchNormalization
from tensorflow.keras.optimizers import Adam
from tensorflow.keras.models import Model
from tensorflow.keras.initializers import RandomNormal
from functools import partial
from src.losses import *
from src.SeparableConv3D import DepthwiseConv3D
...
class ReflectionPadding3D(Layer):
    ...
    def call(self, x, mask=None):
        w_pad,h_pad,pad = self.padding
        return tf.pad(x, [[0,0],[pad,pad], [h_pad,h_pad], [w_pad,w_pad], [0,0] ], 'REFLECT')
def novel_residual_block(X_input, filters,base):

    name_base = base + '/branch'
    X = X_input
    X = ReflectionPadding3D((1,1,1),name=name_base + '1/rf')(X)
    X = DepthwiseConv3D( kernel_size=(3,3,3), strides=(1,1,1),dilation_rate=(1,1,1), padding='VALID',name=name_base + '1/sepconv')(X)
    ...
    ## Branch 1 ext1
    X_branch_1 = ReflectionPadding3D((1,1,1),name=name_base + '1_1/rf')(X)
    X_branch_1 = DepthwiseConv3D( kernel_size=(3,3,3), strides=(1,1,1), padding='VALID',name=name_base + '1_1/sepconv')(X_branch_1)
    ...
    ## Branch 2
    X_branch_2 = ReflectionPadding3D((1,1,1),name=name_base + '2/rf')(X)
    X_branch_2 = DepthwiseConv3D(kernel_size=(3,3,3), strides=(1,1,1), dilation_rate=(1,1,1), padding='VALID',name=name_base + '2/sepconv')(X_branch_2)
...
def disc_res_block(X_input, filters,base):
    ...
    ## Branch 1
    X_branch_1 = ReflectionPadding3D((1,1,1),name=name_base + '1_1/rf')(X)
    X_branch_1 = Conv3D(filters, kernel_size=(3,3,3), strides=(1,1,1), dilation_rate=(1,1,1), padding='valid',kernel_initializer=RandomNormal(stddev=0.02),name=name_base + '1_1/conv')(X_branch_1)
    X_branch_1 = BatchNormalization(axis=3, center=True, scale=True, name=name_base + '1_1/BNorm')(X_branch_1)
    X_branch_1 = LeakyReLU(alpha=0.2,name=name_base + '1_1/LeakyRelu')(X_branch_1)

    ## Branch 2
    X_branch_2 = ReflectionPadding3D((1,1,1),name=name_base + '1_2/rf')(X)
    X_branch_2 = DepthwiseConv3D(kernel_size=(3,3,3), strides=(1,1,1), dilation_rate=(1,1,1), padding='VALID',kernel_initializer=RandomNormal(stddev=0.02),name=name_base + '1_2/sepconv')(X_branch_2)
...
def SFA(X,filters,i):
    ...
    X = DepthwiseConv3D( kernel_size=(1,1,1), strides=(1,1,1), padding='SAME',kernel_initializer=RandomNormal(stddev=0.02),name="SFA_"+str(i+1)+"/conv1")(X)
    ...
    X = Conv3D(filters, kernel_size=(3,3,3), strides=(1,1,1), padding='same',kernel_initializer=RandomNormal(stddev=0.02),name="SFA_"+str(i+1)+"/conv2")(X)
...
def encoder_block(X,down_filters,i):
    X = Conv3D(down_filters, kernel_size=(4,4,4), strides=(2,2,2), padding='same',kernel_initializer=RandomNormal(stddev=0.02),name="down_conv_"+str(i+1))(X)
...
def decoder_block(X,up_filters,i):
    ...
    X = Conv3DTranspose(filters=up_filters, kernel_size=(6,6,6), strides=(2,2,2), padding='same',kernel_initializer=RandomNormal(stddev=0.02),name="up_convtranpose_"+str(i+1) )(X)
    X = BatchNormalization(name="up_bn_"+str(i+1))(X)
    X = LeakyReLU(alpha=0.2,name="up_leakyRelu_"+str(i+1))(X)

    return X

def coarse_generator(img_shape=(32,32,32,4),ncf=64, n_downsampling=2, n_blocks=9, n_channels=1):
    X_input = Input(img_shape,name="input")

    X = ReflectionPadding3D((3,3,3))(X_input)
    X = Conv3D(ncf, kernel_size=(7,7,7), strides=(1,1,1), padding='valid',kernel_initializer=RandomNormal(stddev=0.02),name="conv1")(X)
    ...
    X = ReflectionPadding3D((3,3,3),name="final/rf")(X_up2_add)
    X = Conv3D(n_channels, kernel_size=(7,7,7), strides=(1,1,1), padding='valid',kernel_initializer=RandomNormal(stddev=0.02),name="final/conv")(X)
...
def fine_generator(x_coarse_shape=(32,32,32,64),input_shape=(512, 512, 3), nff=64, n_blocks=3, n_coarse_gen=1,n_channels = 1):
    ...
    for i in range(1, n_coarse_gen+1):
        ...
        X = ReflectionPadding3D((3,3,3),name="rf_"+str(i))(X_input)
        X = Conv3D(down_filters, kernel_size=(7,7,7), strides=(1,1,1), padding='VALID',kernel_initializer=RandomNormal(stddev=0.02),name="conv_"+str(i))(X)
        ...
        print(X)
        X = DepthwiseConv3D( kernel_size=(1,1,1), strides=(1,1,1), padding='SAME',kernel_initializer=RandomNormal(stddev=0.02),name="sepconv_"+str(i))(X)
    ...
    X = ReflectionPadding3D((3,3,3),name="final/rf")(X_up1_add)
    X = Conv3D(n_channels, kernel_size=(7,7,7), strides=(1,1,1), padding='valid',name="final/conv")(X)
    X = Activation('tanh',name="tanh")(X)
...
def discriminator_ae(input_shape_fundus=(512, 512, 3),
    ...
    X = Conv3D(1, kernel_size=(4,4,4), strides=(1,1,1), padding='same',kernel_initializer=RandomNormal(stddev=0.02))(X_up3)
...
def RVgan(g_model_fine,g_model_coarse, d_model1, d_model2,image_shape_fine,image_shape_coarse,image_shape_x_coarse,label_shape_fine,label_shape_coarse,inner_weight = None):
